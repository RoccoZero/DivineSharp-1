...
namespace RockHeroes.Modules
{
    internal class AutoHook
    {
        ...
        private void OnIngameUpdate()
        {
            ...
            var target = EntityManager.GetEntities<Hero>().Where(x => x.Distance2D(GameManager.MousePosition) < 800 && x.IsVisible && !x.IsAlly(localHero) && x.IsAlive && !x.IsIllusion).OrderBy(x => x.Distance2D(GameManager.MousePosition)).FirstOrDefault();
            if (target == null)
            {
                return;
            }
