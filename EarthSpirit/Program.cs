...
namespace EarthSpirit
{
    class Earth_Spirit : Bootstrapper
    {
        ...
        private readonly InputManagerKeyEventHandler ctrl;
        ...
        private static bool HasStoneInRadius(Hero myHero, Vector3 pos, float radius)
        {
            ...
            bool stones = false;
            stones = EntityManager.GetEntities<Entity>().Where(x => x.Name == "npc_dota_earth_spirit_stone" && x.Distance2D(myHero.Position) < radius).Any();
            if (stones == true)
                return true;
            return false;
        }
        ...
        private void OnUpdate()
        {
            ...
            if (kick_ready && GameManager.GameTime > enchant_time && enchant_time > 0)
            {
                enchant_time = 0;
                Tower nearest2HeroTower = GetNearestAlliedTowerToMyHero(myHero);
                if (nearest2HeroTower != null || nearest2HeroTower != default)
                {
                    float kick_ground_target_range = 1600;
                    //Vector3 NormalizedMyHero = my_hero_pos.Normalized();
                    //Vector3 target = my_hero_pos + nearest2HeroTower.Position - NormalizedMyHero * kick_ground_target_range;
                    Vector3 target = ;
                    kick.Cast(target);
                }
            }
            ...
            if (inRange == true)
            {
                ...
                if (kick_ready && GameManager.GameTime > enchant_time && GameManager.GameTime > kick_time)
                {
                    Hero enemy = null;
                    Tower tower = null;
                    enemy = EntityManager.GetEntities<Hero>().Where(x => x.IsEnemy(myHero) && x.IsVisible && x.IsValid && x.Distance2D(my_hero_pos) < 250).OrderBy(x => x.Distance2D(myHero)).FirstOrDefault();
                    ...
                    if (tower != null && enemy != null)
                    {
                        ...
                        else if (myHero.Distance2D(goodPoint) < 100)
                        {
                            ...
                            {
                                kick.Cast(enemy);
                                kick_time = GameManager.GameTime + 0.05f;
                            }
                        }
                    }
                }
                ...
                if (kick_ready && GameManager.GameTime > enchant_time && GameManager.GameTime > kick_time)
                {
                    Hero enemy = null;
                    Hero hero = null;
                    enemy = EntityManager.GetEntities<Hero>().Where(x => x.IsEnemy(myHero) && x.Distance2D(my_hero_pos) < 250 && x.IsVisible && x.IsAlive).OrderBy(x => x.Distance2D(my_hero_pos)).FirstOrDefault();
                    hero = EntityManager.GetEntities<Hero>().Where(x => x.Position != my_hero_pos && x.Health > 0.7 && x.IsAlly(myHero) && x.IsVisible && x.IsAlive && myHero.Distance2D(x) < 1800).OrderByDescending(x => x.Health).FirstOrDefault();
